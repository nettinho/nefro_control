defmodule NefroControlWeb.CoreComponents do
  @moduledoc """
  ...
  import NefroControlWeb.Gettext
    ...
    >
      ...
      >
        <div class="flex min-h-screen items-center justify-center">
          <div class="w-full max-w-3xl p-1 md:p-6 lg:py-8">
            <.focus_wrap
              id={"#{@id}-container"}
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div class="relative" phx-feedback-for={@name}>
      <.label for={@id}><%= @label %></.label>
      <textarea id={@id} name={@name} class={~w(
          block w-full
          px-2.5 pb-2.5 pt-4
          bg-transparent
          rounded-lg border-1 border-gray-300  dark:border-gray-600
  ...
  def input(assigns) do
    ...
    <div phx-feedback-for={@name} class="relative">
      <input type={@type} name={@name} id={@id} class={~w(
      block w-full
      px-2.5 pb-2.5 pt-4
      bg-transparent
      rounded-lg border-1 border-gray-300  dark:border-gray-600
      ...
      focus:outline-none focus:ring-0 focus:border-blue-600 dark:focus:border-blue-500
      appearance-none peer
    )} value={Phoenix.HTML.Form.normalize_value(@type, @value)} {@rest} />
      <.label for={@id}><%= @label %></.label>
    </div>
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class={~w(
        absolute px-2 top-2 z-10 origin-[0] start-1
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(NefroControlWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(NefroControlWeb.Gettext, "errors", msg, opts)
    end
  end
